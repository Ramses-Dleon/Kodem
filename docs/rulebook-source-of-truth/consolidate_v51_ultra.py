#!/usr/bin/env python3
"""
Consolidation v5.1 FINAL-ULTRA — incorporates spot-check pass 5 for p19
with 6 segmented images validated by Ramsés.

Changes vs consolidate_v51_final.py:
  - p19 now has `spot_check_pass == 5` and `spot_check_status == "COMPLETED"`
    (was pass 4).
  - Diagram has 52 nodes / 58 edges (was 51/57) — new node discovered,
    multiple shape corrections, Veneno → rosa (not amarillo), typo "Exinción"
    preserved verbatim, Revelar = amarillo óvalo, INICIO DEL TURNO RIVAL
    added as gray rectangle transition node.
  - Stats include `spot_check_pass5_count: 1` and `total_corrections_p19: 9`
    (3 pass 4 + 6 pass 5).
  - TOC marker for p19 remains ✅ (pass 5 = final canonical).
  - Legend adds ✅ Pass 5 entry (imágenes segmentadas validadas por Ramsés).
...
METHODOLOGY = (
    "Pipeline de 4 capas + post-validación iterativa: (1) Extracción LLM (opus-4-7) "
    ...
    "canónica con 51 nodos y 57 edges. (4.c) Pass 5 FINAL-ULTRA sobre p19: Ramsés envió "
    "6 imágenes segmentadas (una por fase) que permitieron 6 correcciones adicionales: "
    "Veneno = rosa (no amarillo), múltiples shapes a óvalo, typo 'Exinción' preservado "
    "verbatim, Revelar = amarillo óvalo, INICIO DEL TURNO RIVAL agregado como nodo gris "
    "rectángulo, nuevo nodo 'Sólo 1 costo pagado' descubierto en Pasar Turno → estructura "
    "canónica final con 52 nodos y 58 edges (confidence_final 0.98). Consolidación usa "
    "LLM como fuente canónica (descripciones de figuras más ricas) preservando typos "
    "verbatim detectados por árbitro y humano."
)

# Pages reviewed in pass 3 (p19 promoted through pass 4 → pass 5 final).
SPOT_CHECK_PASS3_PAGES = {4, 5, 6, 14, 32, 36, 40}
SPOT_CHECK_PASS5_PAGES = {19}
ALL_SPOT_CHECK_PAGES = SPOT_CHECK_PASS3_PAGES | SPOT_CHECK_PASS5_PAGES

# Total corrections applied to p19 across pass 4 + pass 5 (3 + 6 = 9).
TOTAL_CORRECTIONS_P19 = 9
...
def render_p19_canonical_diagram(diagram):
    """Render the canonical turn-structure diagram for p19 (pass 5 FINAL-ULTRA)."""
    if not diagram:
        return ""

    # Count nodes and edges for the header stats
    phases = diagram.get('phases') or {}
    total_nodes = 0
    total_edges = 0
    for key in PHASE_ORDER:
        if key in phases:
            total_nodes += len(phases[key].get('nodes') or [])
            total_edges += len(phases[key].get('edges') or [])

    out = []
    out.append("## Diagrama Estructura de Turno (CANONICAL_FINAL)")
    out.append("")
    out.append(
        f"> ✅ **Pass 5: imágenes segmentadas validadas por Ramsés** — "
        f"Estructura canónica FINAL-ULTRA. "
        f"**{total_nodes} nodos · {total_edges} edges · 6 fases.** "
        f"Referencia: [`p19-extraction-v3-final.json`](./p19-extraction-v3-final.json). "
        f"Validada por 3 jueces Opus 4.7 + spot-check humano (Ramsés) + "
        f"6 imágenes segmentadas (una por fase). `confidence_final = 0.98`."
    )
    out.append("")
    out.append(
        "> 🎯 **Corrección crítica pass 5:** **Veneno = ROSA (acción), no amarillo** — "
        "confirmado por imagen segmentada de Fin de Turno. Esta corrección invalida "
        "la categorización previa como 'efectos del rival' y reubica Veneno como "
    ...
    # Fases — summary table of counts
    if phases:
        out.append("### Resumen por fase")
        out.append("")
        out.append("| Fase | Nodos | Edges |")
        out.append("|---|---:|---:|")
        for key in PHASE_ORDER:
            ...
            n = len(ph.get('nodes') or [])
            e = len(ph.get('edges') or [])
            out.append(f"| {name} | {n} | {e} |")
        out.append(f"| **TOTAL** | **{total_nodes}** | **{total_edges}** |")
        out.append("")

    # Fases — nodos y conexiones detallados
    if phases:
        ...
        for key in PHASE_ORDER:
            ...
            # Fase Post special block — typo Exinción preservado verbatim
            if key == "fase_post":
                out.append(
                    "> 🔤 **Typo verbatim preservado:** el PDF escribe "
                    "**«Exinción»** (sin la _c_) en el nodo «Cartas con 0 ptos. de "
                    "vida van a Exinción». Se preserva tal cual aparece en el fuente."
                )
                out.append("")
            ...
            if nodes:
                ...
                for idx, n in enumerate(nodes, 1):
                    ...
                    typo_note = (n.get('typo_note') or '').replace("|", "/")
                    correction_note = (n.get('correction_note') or '').replace("|", "/")
                    extra_parts = []
                    if shape:
                    ...
                    if typo_note:
                        extra_parts.append(f"⚠️ {typo_note}")
                    if correction_note:
                        extra_parts.append(f"✅ {correction_note}")
            ...
            if pn:
                ...
                out.append("")
            # P5 Ramsés note (if any)
            rn = (ph.get('note_ramses_p5') or '').strip()
            if rn:
                out.append(f"_Nota Ramsés (pass 5):_ {rn}")
                out.append("")
    ...
    # Nota sobre Pasar Turno como flujo paralelo
    ...
    # Log de correcciones pass 4 + pass 5
    corrections_log = diagram.get('corrections_log') if isinstance(diagram, dict) else None
    # corrections_log lives at the top-level of p19 json, not diagram_structure
    return "\n".join(out)


def render_p19_corrections_log(llm):
    """Render the pass 4 + pass 5 corrections log for p19."""
    log = llm.get('corrections_log') or []
    if not log:
        return ""
    out = []
    out.append("### Log de correcciones (pass 4 + pass 5)")
    out.append("")
    out.append(
        f"Total correcciones aplicadas a p19 tras la extracción inicial: "
        f"**{TOTAL_CORRECTIONS_P19}** (3 en pass 4 + 6 en pass 5)."
    )
    out.append("")
    for entry in log:
        date = entry.get('date', '')
...
def build_page_md(page_num, llm, arb):
    ...
    if sc_pass in (3, 4, 5):
        if sc_pass == 5 and sc_status == "COMPLETED":
            out.append(
                "> ✅ **Pass 5: imágenes segmentadas validadas por Ramsés** — "
                "CANONICAL_FINAL. Extracción FINAL-ULTRA consolidada humano + LLM + "
                "3 jueces + 6 imágenes segmentadas (una por fase). "
                "`confidence_final = 0.98`."
            )
        elif sc_pass == 4 and sc_status == "COMPLETED":
            out.append(
    ...
    if sc_pass in (4, 5) and llm.get('confidence_final') is not None:
        badge_parts.append(f"**Confianza final (pass {sc_pass}):** {llm.get('confidence_final')}")
    ...
    if page_num == 19:
        ...
        # Append corrections log after diagram
        log_md = render_p19_corrections_log(llm)
        if log_md:
            out.append(log_md)
...
def build_toc(pages_meta):
    ...
    for m in pages_meta:
        ...
        if sc_pass in (4, 5) and sc_status == "COMPLETED":
            badges.append("✅")
        elif sc_pass == 3:
...
def main():
    ...
    for i in range(1, 41):
        ...
        if sc_pass == 3:
            ...
            spot_check_pass5_count += 1
            pass5_pages.append(i)
        ...
        if llm.get('corrections_log'):
            page_entry["corrections_log"] = llm.get('corrections_log')
    ...
    if spot_check_pass4_count:
        md_parts.append(
            f"**Post-validación conjunta pass 4:** {spot_check_pass4_count} página(s) → "
            f"{', '.join(f'p{p:02d}' for p in pass4_pages)} ✅  "
        )
    md_parts.append(
        f"**Pass 5 FINAL-ULTRA (imágenes segmentadas):** {spot_check_pass5_count} página(s) → "
        f"{', '.join(f'p{p:02d}' for p in pass5_pages)} ✅  "
    )
    md_parts.append(
        f"**Total correcciones aplicadas a p19:** {TOTAL_CORRECTIONS_P19} "
        f"(3 en pass 4 + 6 en pass 5)  "
    )
    if post_val_pending:
        md_parts.append(
        ...
        md_parts.append("**Post-validación pendiente:** ninguna (p19 consolidada en pass 5 FINAL-ULTRA) ✅  ")
    ...
    md_parts.append("- ✅ Post-validación conjunta completada (pass 4 o pass 5 FINAL-ULTRA)")
    md_parts.append("")
    ...
    json_doc = {
        ...
        "stats": {
            ...
            "spot_check_pass5_count": spot_check_pass5_count,
            "spot_check_pass5_pages": pass5_pages,
            "total_corrections_p19": TOTAL_CORRECTIONS_P19,
    ...
    assert parsed['stats']['spot_check_pass5_count'] == 1, \
        f"Expected pass5_count=1, got {parsed['stats']['spot_check_pass5_count']}"
    assert parsed['stats']['total_corrections_p19'] == TOTAL_CORRECTIONS_P19
    # Verify p19 has canonical diagram inlined with 52 nodes / 58 edges
    ...
    assert p19_entry['spot_check_pass'] == 5
    assert p19_entry['spot_check_status'] == "COMPLETED"
    assert p19_entry.get('confidence_final') == 0.98

    # Count p19 nodes/edges for verification
    p19_phases = p19_entry['diagram_structure'].get('phases') or {}
    tn = sum(len(ph.get('nodes') or []) for ph in p19_phases.values())
    te = sum(len(ph.get('edges') or []) for ph in p19_phases.values())
    assert tn == 52, f"Expected 52 nodes, got {tn}"
    assert te == 58, f"Expected 58 edges, got {te}"
    ...
    manifest = {
        ...
        "stats": {
            ...
            "spot_check_pass5_count": spot_check_pass5_count,
            "total_corrections_p19": TOTAL_CORRECTIONS_P19,
            "p19_diagram_nodes": tn,
            "p19_diagram_edges": te,
        ...
        "files": [
            {
                ...
                "description": ("Documento legible humano con TOC, bloques, figuras, "
                                "spot-check pass 3/4/5 (p19 CANONICAL_FINAL FINAL-ULTRA "
                                "con 52 nodos/58 edges) y apéndices."),
            },
            {
                ...
                "description": ("JSON estructurado con páginas, anchors_index, typos, "
                                "diagram_structure canónica pass 5 (p19: 52 nodos/58 edges), "
                                "corrections_log (9 correcciones totales en p19), "
    ...
    assert man_parsed['stats']['spot_check_pass5_count'] == 1
    assert man_parsed['stats']['total_corrections_p19'] == TOTAL_CORRECTIONS_P19
    assert man_parsed['stats']['p19_diagram_nodes'] == 52
    assert man_parsed['stats']['p19_diagram_edges'] == 58

    OUT_MANIFEST.write_text(man_str, encoding='utf-8')

    elapsed = time.time() - start_time
    print(f"\n=== Re-consolidación v5.1 FINAL-ULTRA completada ===")
    ...
    print(f"  Spot-check pass 5      : {spot_check_pass5_count} → {pass5_pages} ✅ FINAL-ULTRA")
    print(f"  Correcciones p19       : {TOTAL_CORRECTIONS_P19} (3 pass4 + 6 pass5)")
    print(f"  p19 diagram            : {tn} nodos / {te} edges")
